...
class LanguageModel(object):
    ...
    def _build(self):
        ...
        for lstm_num, lstm_input in enumerate(lstm_inputs):
            ...
            self.lstm_outputs.append(_lstm_output_unpacked)

        self.flat_lstm_outputs = [tf.reshape(elem, [-1, projection_dim]) for elem in self.lstm_outputs]
        self._build_loss(self.flat_lstm_outputs)
        self.output_softmaxes = [tf.reshape(elem, [batch_size, -1, n_tokens_vocab]) for elem in self.flat_output_softmaxes]
